<div class="box pt-2">
    <form method="POST" action="<?= isset($postCatalogue) ? ('dashboard/post_catalogues/update/' . $postCatalogue['id']) : 'dashboard/post_catalogues/store'?>">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select name="parent_id" class="form-control">
                            ...
                            <?php foreach ($postCatalogues as $val) { ?>
                                <option value="<?= $val['id'] ?>"
                                    <?= (isset($postCatalogue['parent_id']) && $postCatalogue['parent_id'] == $val['id'])
                                        ? 'selected'
                    ...
                    <div class="form-group col-md-12 bg-light text-center">
                        <span class="image img-cover">
                            <img id="ckAvataImg" width="150px" class="image-target"
                                 src="<?= isset($postCatalogue['image']) && $postCatalogue['image'] ? $postCatalogue['image'] : (!empty(old('image')) ? old('image') : 'public/assets/images/no-image.jpg') ?>"
                                 alt="no images">
                        </span>
                        <input type="hidden" id="ckAvata" class="ck-target" name="image" value="<?=isset($postCatalogue['image']) && $postCatalogue['image'] ? $postCatalogue['image'] : (old('image') ?? '')?>">
                    </div>
                </div>
