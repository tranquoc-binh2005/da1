<div class="box pt-2">
    <form method="POST" action="<?= isset($productCatalogue) ? ('dashboard/product_catalogues/update/' . $productCatalogue['id']) : 'dashboard/product_catalogues/store'?>">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select name="parent_id" class="form-control">
                            ...
                            <?php foreach ($productCatalogues as $val) { ?>
                                <option value="<?= $val['id'] ?>"
                                    <?= (isset($productCatalogue['parent_id']) && $productCatalogue['parent_id'] == $val['id'])
                                        ? 'selected'
                                        : ((old('parent_id') == $val['id']) ? 'selected' : '') ?>>
                    ...
                    <div class="form-group col-md-12 bg-light text-center">
                        <span class="image img-cover">
                            <img id="ckAvataImg" width="150px" class="image-target"
                                 src="<?= isset($productCatalogue['image']) && $productCatalogue['image'] ? $productCatalogue['image'] : (old('image') ?? 'public/assets/images/no-image.jpg') ?>"
                                 alt="no images">
                        </span>
                        <input type="hidden" id="ckAvata" class="ck-target" name="image" value="<?=isset($productCatalogue['image']) && $productCatalogue['image'] ? $productCatalogue['image'] : (old('image') ?? '')?>">
                    </div>
                </div>
