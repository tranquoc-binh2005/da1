...
<div class="card-box">
    <form action="" method="GET">
        <div class="filter-box">
            <a class="btn btn-info" href="/dashboard/product_catalogues/create">Thêm mới</a>
    ...
    <div class="table-responsive">
        <table class="table mb-0">
            ...
            <tbody>
            <?php if(empty($productCatalogues['data'])){?>
                <tr>
                    <td class="text-center" colspan="5">Không có bản ghi phù hợp</td>
                </tr>
            <?php }?>
            <?php foreach ($productCatalogues['data'] as $productCatalogue) {?>
            <tr>
                <td class="text-left" width="350px">
                    <?= str_repeat('|--', $productCatalogue['level'] - 1) . ' ' . $productCatalogue['name']?>
                </td>
                <td>Người tạo: <?= $productCatalogue['admin_name'] ?: 'Chưa cập nhật' ?> <br>
                    <?= $productCatalogue['description'] ?: 'Chưa cập nhật' ?>
                </td>
                <td class="ol-info">
                    <img class="image" src="<?= $productCatalogue['image']?>" alt="<?= $productCatalogue['name']?>">
                </td>
                <td class="ol-info">
                    <input type="number"
                           class="inputOrderTop"
                           name="order"
                           value="<?=$productCatalogue['order']?>"
                           data-field="product_catalogues"
                           data-id="<?=$productCatalogue['id']?>"
                           data-column="`order`"
                    >
                </td>
                <th class="text-center">
                    <input type="checkbox"
                           ...
                           data-id="<?=$productCatalogue['id']?>"
                           data-column="publish"
                           data-publish="<?=$productCatalogue['publish']?>">
                </th>
                <th class="text-center">
                    <a style="font-size: 20px;" href="/dashboard/product_catalogues/show/<?=$productCatalogue['id']?>"><i class="fe-edit"></i></a>
                    <a style="font-size: 20px; color:rgb(241, 55, 55);" href="/dashboard/product_catalogues/delete/<?=$productCatalogue['id']?>"><i class="fe-trash"></i></a>
                </th>
            </tr>
            <tr>
            </tr>
