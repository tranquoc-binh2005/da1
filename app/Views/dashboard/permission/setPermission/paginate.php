...
<ul class="pagination pagination-rounded text-right">
    <li class="paginate_button page-item <?= $currentPage == 1 ? 'disabled' : '' ?>">
        <a href="/dashboard/permissions/bulkStore?<?= $queryString ?>page=<?= max(1, $currentPage - 1) ?>" class="page-link">
    ...
    echo '<li class="paginate_button page-item ' . ($currentPage == 1 ? 'active' : '') . '">
            <a href="/dashboard/permissions/bulkStore?' . $queryString . 'page=1" class="page-link">1</a>
          </li>';
    ...
    for ($i = $start; $i <= $end; $i++) {
        echo '<li class="paginate_button page-item ' . ($currentPage == $i ? 'active' : '') . '">
                <a href="/dashboard/permissions/bulkStore?' . $queryString . 'page=' . $i . '" class="page-link">' . $i . '</a>
              </li>';
    }
    ...
    if ($totalPages > 1) {
        echo '<li class="paginate_button page-item ' . ($currentPage == $totalPages ? 'active' : '') . '">
                <a href="/dashboard/permissions/bulkStore?' . $queryString . 'page=' . $totalPages . '" class="page-link">' . $totalPages . '</a>
              </li>';
    }
    ?>

    <li class="paginate_button page-item <?= $currentPage == $totalPages ? 'disabled' : '' ?>">
        <a href="/dashboard/permissions/bulkStore?<?= $queryString ?>page=<?= min($totalPages, $currentPage + 1) ?>" class="page-link">
